...
public class StreamTest {
    ...
    public static List<KeyValue<String, WeatherAlert>> main(String[] args) throws InterruptedException, IOException {
        if (args.length != 2) {
        ...
        KStream<String, WeatherAlert> weather = lines.flatMap((key, value) -> {
            List<KeyValue<String,WeatherAlert>> tmp = new ArrayList<KeyValue<String,WeatherAlert>>();
            try {
                WeatherAlert alert = new WeatherAlert(value.getString("type"), value.getString("location"));
                tmp.add(new KeyValue(key, alert));
            } catch (JSONException e) {
                // Handle the error here
            }
            return tmp;
        }, Serdes.String(), new WeatherAlertSerde());
